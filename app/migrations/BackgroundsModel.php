...
/*
    Asatru PHP - Migration for BackgroundsModel
*/

/**
 * This class specifies a migration
 */
class BackgroundsModel_Migration {
    ...
    public function up()
    {
        $this->database = new Asatru\Database\Migration('tbl_backgrounds', $this->connection);
        $this->database->drop();
        $this->database->add('id INT NOT NULL AUTO_INCREMENT PRIMARY KEY');
        $this->database->add('name VARCHAR(512) NOT NULL');
        $this->database->add('description VARCHAR(1024) NULL');
        $this->database->add('author VARCHAR(512) NOT NULL');
